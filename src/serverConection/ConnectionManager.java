...
public class ConnectionManager {
	...
	public void login(String username, String password) throws AuthenticationError {
		try {
			...
			connection = prepareHttpURLConection(request, parameters, "POST", "application/json; charset=UTF-8");
			...
			if (HttpResult == HttpURLConnection.HTTP_OK) {
				try (InputStream is = connection.getInputStream(); JsonReader rdr = Json.createReader(is)) {// try
	...
	/**
	 * This method asks for articles that can be shown to the user logged or to any
	 * user
	 * 
	...
	public List<Article> getArticles() throws ServerCommunicationError, IOException {
		...
		HttpURLConnection connection = this.getHttpURLConection(request, parameters, "GET",
	...
	/**
	 * This method deletes the given article, if only if the logged user is the
	 * article's author
	 * 
	 * @param idArticle
	 * @throws ServerCommunicationError
	...
	public void deleteArticle(int idArticle) throws ServerCommunicationError {
		try {
			...
			String request = serviceUrl + "article/" + idArticle;
			HttpURLConnection connection;
			connection = this.getHttpURLConection(request, parameters, "DELETE",
					"application/x-www-form-urlencoded; charset=UTF-8");
			if (connection != null) {
	...
	// Auxiliary services
	private HttpURLConnection getHttpURLConection(String request, String parameters, String requestMethd,
			String contentType) {
		...
		try {
			...
			int HttpResult = connection.getResponseCode();
			if (HttpResult != HttpURLConnection.HTTP_OK && HttpResult != HttpURLConnection.HTTP_NO_CONTENT) {// IF OK
	...
	private HttpURLConnection prepareHttpURLConection(String request, String parameters, String requestMethd,
			String contentType)
